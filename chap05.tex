...
формуле~\cite{Millero:1973}, используя алгоритмы~\cite{Fofonoff:1983}. 
...
около~$\degrees{30}$~\cite[стр.~43]{Stewart:1985}.}
...
проекта по спутниковой климатологии~\cite{Darnell:1992}.}
\label{fig:QswDown}
\end{figure}
%
% \begin{figure}[t!]
...
достигающий максимума в этом диапазоне~\cite{Selby:1975}.}
...
достаточно простом изложении доступно в работе~\cite[стр.~24--26]{Hartmann:1994}.
...
В работе~\cite[стр.~39--41, 43--44, 605--607]{Gill:1982} приводятся уравнения,
...
изменчивости~\cite{Darnell:1988}, \cite{Rossow:1991}.
%% ISCCP
%
...
в некоторых современных работах, таких как~\cite{Cess:1995} и~\cite{Ramanathan:1995},
...
\begin{paragraph}{Атмосферные осадки.}
...
атмосферной циркуляции~\cite{Schubert:1993}, 
либо путем объединения судовых и спутниковых наблюдений с результатами 
численных моделей прогноза погоды~\cite{Xie:1997}.
%
...
\begin{paragraph}{Испарение (поток скрытого тепла).}
...
информации об океане. В работе~\cite{Slutz:1985} усилия по сбору, 
...
океаном и атмосферой. Наблюдения каждого такого судна были попутными,
то есть их место и время определялись основной целью плавания (торговой,
военной), а не научными соображениями. Информация заносилась в судовые 
журналы, откуда затем попадала в организации, занимающиеся ее анализом.
Впоследствии к ней добавились данные со специализированных 
исследовательских судов, буев и других устройств. Сведения о состоянии
погоды и морской поверхности накапливались,
...
атмосфере и океану (ИКОАДС)~\cite{Woodruff:1987}, распространяемый
...
В работе~\cite{Gleckler:1997} дается анализ точности данных ИКОАДС
...
буях установило, что входящий в океан поток, осредненный 
по всей его поверхности, имеет погрешность~$\pm 30\wpsqm$~\cite{Josey:1999}. Величина 
...
предложена в работе~\cite{DaSilva:1995} и показана 
...
потоков инфракрасной радиации с земной поверхности~\cite{Rossow:1991}.
...
осадков~\cite{Huffman:1995}, \cite{Huffman:1997}:
%
...
по данным реанализа НЦПОС/НКАР~\cite{Xie:1997}. Этот комплект данных имеет то же самое
...
ниже~\cite[стр.~258]{Taylor:2000}. Chou et al обнаружены большие различия в
величинах потоков, рассчитанных разными исследовательскими коллективами~\cite{Chou:2004}.
%
...
не в состоянии адекватно описать структуру облачности низкого уровня~\cite{Taylor:2001}.
%
...
тепла~$5.8\wpsqm$~\cite[стр.~206]{Taylor:2000}. По данным ИКОАДС, входящий в океан
...
южного полушария. В целом, по мнению Тейлора не существует
идеальных комплектов данных; все они содержат существенные и неизвестные
ошибки~\cite{Taylor:2000}.
%
...
работе~\cite[стр.~58]{Houghton:1996}, которая, в свою очередь, основана на
данных из работы~\cite{Kiehl:1996}.}
...
тепла~$Q_L$, вычисленные на основе комплекта данных ИКОАДС~\cite{DaSilva:1995}.
...
(Исследовательский центр Лэнгли, НАСА)~\cite{Darnell:1992}
...
данных ИКОАДС~\cite{DaSilva:1995}. Изолинии проведены с 
...
максимален в регионах с наименьшей облачностью. Этот поток везде отрицателен.
...
с континентов.\remark{То есть, более значительны зимой.} 
...
Вычисления Trenberth and Caron показали, что суммарный среднегодовой
меридиональный перенос тепла океаном и атмосферой достигает пикового 
значения~$6\petawatt$ на широтах порядка~$\degrees{35}$~\cite{Trenberth:2001}.
%
...
(Согласно работе~\cite[стр.~212]{Houghton:1996}, в которой использованы 
данные~\cite{Trenberth:1994}.) 
...
в северном и 8\% в южном полушариях, соответственно~\cite{Trenberth:2001}.
%
...
излучение может варьировать в течение столетий на~$\pm 0.2\%$~\cite{Lean:1995}, 
...
последнего столетия были обнаружены небольшие периодические (12~лет, 22~года и др.) 
изменения поверхностной температуры воды~\cite{White:1998}. Наблюдаемая
