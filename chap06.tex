...
точностью,  оказалась непростым делом~\cite{Lewis:1980}, так что в разные моменты
...
двумерный класс~$\degCent{0.1} \times 0.01$.~\cite[стр.~47]{Worthington:1981}}
...
поскольку на его последних стадиях теряются хлориды~\cite[стр.~50]{Sverdrup:1942}.
%
...
cолёность~$S$ была определена\remark{Эта формула известна как <<формула Кнудсена>>.} 
через хлорность:
...
соотношение\remark{Также известное как <<формула ЮНЕСКО>>.} 
между солёностью и хлорностью~\cite{Wooster:1969}:
...
<<Копенгагенской>> воды. В работе~\cite{Millero:1996} подчеркивается, что 
...
К началу 1970-х точные измерители электропроводности уже можно было погружать 
с борта судна для проведения измерений непосредственно на заданной 
глубине.
Необходимость переоценки шкалы солёности привела к тому, что в
1981~г.\ ОГОТС рекомендовала~\cite{JPOTS:1981, Lewis:1980}
...
температуры приводятся в работах~\cite[стр.~72]{Millero:1996} и~\cite{Lewis:1980}.
% 
...
экспедицией <<Челленджера>>~\cite{Dittmar:1884}, и последующих 
исследованиях~\cite{Carritt:1959}.
\begin{quotation}
...
методами, такими как электропроводность\dots~\cite{Sverdrup:1942}
\end{quotation}
Погрешность взаимосвязи электропроводности воды с её солёностью составляет 
$\pm 0.003$~единицы солёности.
Причиной появления столь небольшой погрешности считается
...
Для решения этих проблем было предложено новое определение 
солёности, \emph{эталонная солёность}, которая точно представляет абсолютную
солёность искусственно приготовленного раствора морской воды~\cite{Millero:2008}. В её основе
...
индия($\degCent{156.5985}$)~\cite{Preston-Thomas:1990}. Тройная точка воды~--- 
...
оптимальной интерполяции на основе судовых отчетов
и температурных данных AVHRR~\cite{Reynolds:1995}. 
Изолинии проведены с шагом~$\degCent{1}$ (тонкие)
...
Рейнольдсом и Смитом
в \textit{Climate Diagnostics Bulletin} for February 1995~\cite{Reynolds:1995}.
...
температуре~\cite{Reynolds:1995}. Изолинии проведены с 
...
долгосрочного среднего, малы: они не превышают~$\degCent{1.5}$~\cite{Harrison:1998}, 
...
Годовая изменчивость поверхностной температуры максимальна в умеренных
...
почему солёность Атлантики выше, чем Тихого океана? Брокер 
показал, что $0.32\Sv$~воды, испаряющейся над Атлантическим океаном, не
выпадает в виде осадков на сушу~\cite{Broecker:1997}. 
Вместо этого она переносится ветрами в Тихий
...
поверхности океана~\cite{Levitus:1982}, а также разность испарения и количества 
...
теряет~$0.32\Sv$, объем, сопоставимый со стоком Амазонки.~\cite{Broecker:1997}}
...
в океанологии рекомендовала заменить~$\sigma$ буквой~$\gamma$, 
так как величина~$\sigma$ изначально определялась относительно чистой воды 
и была безразмерной~\cite{SUN:1985}. Однако, в данном пособии мы будем 
следовать устоявшейся практике и использовать обозначение~$\sigma$.
%
...
13 июля 1967~г. под~\latlon{175.825}{E} и~\latlon{28.258}{S}~\cite{Warren:1973}.}
...
как вызвать, так и не вызвать изменение температуры~\cite{McDougall:2003}.
...
возрастание плотности с глубиной. \cite{Lynn:1968}}
...
устранить это затруднение, Джекет и Мак-Дугалл ввели новую величину, 
которую назвали нейтральной плотностью~\cite{Jackett:1997}. 
%
...
название~\emph{нейтрального пути}~\cite{Eden:1999}. 
...
метров в любой точке океана~\cite{Jackett:1997}. 
Этот результат был получен на основе данных из атласа Левитуса~\cite{Levitus:1982}. 
Величины нейтральной плотности, в свою 
...
океанографическим таблицам и стандартам в 1981~г.~\cite{JPOTS:1981}. 
Дополнительная информация доступна в работах~\cite{Millero:1981} 
и~\cite{Millero:1980}. 
...
из трех многочленов с 41 постоянным коэффициентом~\cite{JPOTS:1991}.
...
Инструкцией по производству работ на океанографических станциях,
опубликованной ЮНЕСКО~\cite{JPOTS:1991}. 
Эта книга содержит международно принятые
...
\begin{section}{Измерение температуры}\label{sec:MeasOfTemp}
...
ним температуры окружающей воды. Ртуть расширяется; её количество в
капилляре пропорционально температуре. Затем термометр
...
в период~1900--1970~гг.\ основным источником информации о распределении 
...
океана и верхней границы облаков~\cite{Coakley:1982}. 
...
\begin{enumerate}
\item
Неразличимые или необнаруженные облака: большие толстые
...
градусов~\cite{Emery:1989}. Этот источник погрешности может быть
...
создания карт между~$\pm\degrees{70}$ с точностью~$\pm\degCent{0.6}$~\cite{May:1998}.
Карты температуры поверхности океана пересылаются ВМФ США
...
с использованием метода оптимальной интерполяции Рейнольдса~\cite{Reynolds:2002}. 
...
Pathfinder~\cite{Kilpatrick:2001}.
%
...
во времени и пространстве. Смит и Рейнольдс 
...
или менее после 1950~г.~\cite{Smith:2004}. 
Аномалии поверхностной температуры рассчитывались 
...
вблизи ячейки электропроводности (рис.~\ref{fig:conductivity}). Наилучшая точность, 
...
\caption{Ячейка электропроводности. Через морскую воду проходит электрический ток  
...
Саундерс заметил строгую взаимосвязь температуры и солёности 
большого объема воды, расположенного в глубокой котловине на северо-востоке 
Атлантического океана под Средиземноморским противотечением~\cite{Saunders:1986}. 
Он воспользовался
...
Gouretski и Jancke оценили точность
измерений солёности как функцию времени~\cite{Gouretski:1995}. Используя
...
Gouretski и Jancke методике, составляло~$0.0033$~\cite{Gouretski:1995}.
...
Саундерсом, даёт точность в~$0.002$ (рис.~\ref{fig:911data})~\cite{Saunders:1986}.
%
...
Снодграсс получил прецизионность в~$\pm 0.8\mm$ на 
глубине~$3\km$~\cite{Snodgrass:1964}.
%
...
\ \ \ \ \ \ \\
\caption{\textbf{Слева:} CTD, готовый к спуску за борт судна~\cite{Davis:1987}. 
...
\begin{paragraph}{CTD.}
% \paragraph{CTD}
В начале 1960-х методика сбора сведений о температуре и солёности претерпела
существенные изменения: на смену механическим батометрам с прикрепленными 
к ним термометрами пришел электронный прибор, получивший название~CTD, 
которое подчеркивает его назначение: 
измерение электропроводности, температуры и глубины%
...
электропроводность~--- с помощью ячейки электропроводности, а давление~--- 
...
$5\dBar$~для давления и $0.01$~для солёности~\cite{Riser:2008}.
%
...
батометрических проб высокого качества, собранных на глубинах 
до~$700\m$~\cite{Domingues:2008}.
...
излучения согласно работе~\cite[стр.~18, 19]{Morel:1974}, к которой также следует
...
свет из подповерхностных слоёв.~\cite{Clarke:1970}}
...
соответственно. MODIS измеряет восходящее излучение в 36 диапазонах длин
волн от~$405\nm$ до~$14\,385\nm$.
%
...
Большая часть наблюдаемого со спутника восходящего излучения приходит из
...
Coastal Zone Color Scanner, Gordon et al. предложили~\cite{Gordon:1983}:
\begin{subequations}
\begin{align}
...
\item
Перемешанный слой с постоянной температурой и солёностью представляет 
собой верхний слой океана глубиной~$1$--$100\m$. Конкретная толщина 
